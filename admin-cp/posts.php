        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>'.($post['status'] == 'dreft' ? '<a href="posts.php?status=published&post='.$post['post_id'].'&page='.$page.'" class="btn btn-success btn-sm">نشر</a>' : '<a href="posts.php?status=dreft&post='.$post['post_id'].'&page='.$page.'" class="btn btn-info btn-sm">تعطيل</a>').'</td>
                    <td><a href="edit-post.php?post='.$post['post_id'].'&page='.$page.'" class="btn btn-warning btn-sm">تعديل</a></td>
                    <td><a href="posts.php?delete='.$post['post_id'].'&page='.$page.'" class="btn btn-danger btn-sm">حذف</a></td>
                </tr>
                    ';
                    $num++;
                    }
        ...
        <nav class="text-center">
          <ul class="pagination">
            ...
            for ($i=1; $i <=$total_page ; $i++) {
              echo '<li '.($page == $i ? 'class="active"' :'').'><a href="posts.php?page='.$i.'">'.$i.'</a></i>';
            }
            ?>
          </ul>
        </nav>
