...
if(isset($_POST['edit_post'])){
	...
	if(empty($title)){
		...
		if($image_name != ''){
					 ...
					 if($image_error === 0){
						if($image_size <= 3000000){
							...
							if(move_uploaded_file($image_tmp , $image_dir)){
								$update= mysqli_query($conn, "UPDATE `posts` SET `title` = '$title' , `post` = '$post' , `category` = '$category' , `image` = '$image_dir' , `status` = '$status' WHERE `post_id` = '$_GET[post]' " );
								if(isset($update)){
									$msg =  '<div class="alert alert-success" role="alert">تم تحديث المقال بنجاح , جاري تحويلك الى المقالات</div>';
            ...
            $update= mysqli_query($conn, "UPDATE `posts` SET `title` = '$title' , `post` = '$post' , `category` = '$category' , `status` = '$status' WHERE `post_id` = '$_GET[post]' " );
            if(isset($update)){
                $msg =  '<div class="alert alert-success" role="alert">تم تحديث المقال بنجاح , جاري تحويلك الى المقالات</div>';
                echo '<meta http-equiv="refresh" content="3; \'posts.php\' " />';
            }
		      ...
		      <div class="panel-body">
						<form action="" method="post" class="form-horizontal" enctype="multipart/form-data">
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" value="<?php echo $post['title']; ?>" name="title" id="title" placeholder="أدخل عنوان المقال">
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label for="image" class="col-sm-2 control-label">صورة المقال :</label>
								<div class="col-sm-5">
							...
							<div class="form-group">
								<div class="col-sm-offset-2 col-sm-10">
									<button type="submit" name="edit_post" class="btn btn-danger">تحديث المقال</button>
								</div>
							</div>
						</form>
		      </div>
